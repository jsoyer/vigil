...
class TestSnmpget:
    ...
    def test_returns_none_on_timeout(self):
        with patch(
            "subprocess.run",
            side_effect=subprocess.TimeoutExpired(cmd="snmpget", timeout=5),
        ):
            from snmp_monitor import _snmpget
    ...
    def test_returns_none_when_snmpget_not_found(self):
        with patch(
            "subprocess.run", side_effect=FileNotFoundError("snmpget not found")
        ):
            from snmp_monitor import _snmpget
...
class TestIsStressed:
    ...
    def test_to_dict_all_fields(self):
        ...
        m = UsgMetrics(
            cpu_percent=30.0, memory_percent=50.0, uptime_seconds=3600, reachable=True
        )
        d = m.to_dict()
