...
class TestRunBackup:
    ...
    def test_suspicious_size_returns_error(self, tmp_path):
        ...
        f.write_bytes(b"x" * 100)
        with (
            patch("backup_unifi.UNIFI_BACKUP_DIR", str(tmp_path)),
...
class TestBackupResult:
    def test_summary_ok(self):
        ...
        r = BackupResult(
            ok=True, filename="f.unf", size_bytes=2097152, destination="drive:Unifi"
        )
        s = r.summary()
    ...
    def test_to_dict_no_stale(self):
        ...
        r = BackupResult(
            ok=True, filename="f.unf", size_bytes=1048576, destination="s3:b"
        )
        d = r.to_dict()
        assert d["ok"] is True
