...
class TestIsConfigured:
    def test_returns_true_when_both_set(self):
        with patch("telegram_bot.TELEGRAM_BOT_TOKEN", "tok"), \
...
class TestHandleCommandHelp:
    def test_help_lists_commands(self):
        with patch("telegram_bot._send") as mock_send:
            ...
            for cmd in ["/status", "/pause", "/resume", "/reboot", "/ddns", "/backup", "/help"]:
                assert cmd in msg
...
class TestHandleCommandDdns:
    def test_ddns_not_configured(self):
        with patch("telegram_bot._send") as mock_send:
            with patch.dict("sys.modules", {"ddns_cloudflare": MagicMock(
...
class TestCheckAuth:
    ...
    def test_authorized_chat_dispatches_command(self):
        ...
        with patch("telegram_bot.TELEGRAM_CHAT_ID", "999"), \
    ...
    def test_poll_updates_no_result(self):
        ...
        with patch("telegram_bot._api", return_value=None), \
    ...
    def test_poll_updates_not_ok(self):
        ...
        with patch("telegram_bot._api", return_value={"ok": False}), \
             patch("time.sleep") as mock_sleep:
            bot = TelegramBot(holder)
            bot._poll_updates()
            mock_sleep.assert_called_once_with(5)
...
class TestApiFunction:
    def test_api_returns_parsed_json(self):
        ...
        with patch("telegram_bot.TELEGRAM_BOT_TOKEN", "mytoken"), \
